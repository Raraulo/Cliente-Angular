...
@Component({
  selector: 'app-medicin',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './medicin.component.html',
  styleUrls: ['./medicin.component.css']
})
export class MedicinaComponent implements OnInit {
  ...
  cargarProductos(): void {
    this.productoService.getProductos().subscribe((data: Producto[]) => {
      this.productos = data.filter((p: Producto) => p.animal_id === 7);
    });
  }
  ...
  abrirModalNuevo(): void {
    this.productoSeleccionado = {
      ...
      stock: 0,
      imagen_url: '',
      animal_id: 7, // Fijo para medicina
      disponible: true,
  ...
  guardarNuevoProducto(): void {
    if (this.productoSeleccionado) {
      this.productoSeleccionado.animal_id = 7;
