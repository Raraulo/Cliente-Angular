...
@Component({
  selector: 'app-pequenos',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pequenos.component.html',
  styleUrls: ['./pequenos.component.css']
})
export class PequenosComponent implements OnInit {
  productos: Producto[] = [];
  ...
  cargarProductos(): void {
    this.productoService.getProductos().subscribe((data: Producto[]) => {
      this.productos = data.filter((p: Producto) => p.animal_id === 5); // animales pequeños
    });
  }
  ...
  guardarNuevoProducto(): void {
    if (this.productoSeleccionado) {
      this.productoSeleccionado.animal_id = 5;
